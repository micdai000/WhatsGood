-- Allow businesses in the Other category to store a typed category label.

ALTER TABLE public.businesses
  ADD COLUMN IF NOT EXISTS custom_category TEXT;

ALTER TABLE public.businesses
  DROP CONSTRAINT IF EXISTS businesses_custom_category_length;

ALTER TABLE public.businesses
  ADD CONSTRAINT businesses_custom_category_length
  CHECK (
    custom_category IS NULL
    OR char_length(trim(custom_category)) BETWEEN 2 AND 80
  );

DROP FUNCTION IF EXISTS public.complete_business_onboarding(
