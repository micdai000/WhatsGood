...
type ProfileTab = "activity" | "libraries" | "following";
...
export default function ProfilePage() {
  ...
  const followedEntities = allEntities.filter((e) =>
    followedEntityIds.includes(e.id),
  );
    ...
    <div className="w-full py-6">
      ...
      <div className="mt-6 flex justify-center gap-10">
        {[
          ...
          { value: currentUser.entitiesFollowed, label: "Following" },
        ...
        {activeTab === "following" && (
          <>
            {followedEntities.length > 0 ? (
              <div>
                {followedEntities.map((entity, i) => (
                  <Link
                    key={entity.id}
                    href={`/entity/${entity.id}`}
                    className={
                      "flex items-center gap-3.5 py-3" +
                      (i < followedEntities.length - 1
              ...
              <p className="py-12 text-center text-[14px] text-neutral-400">
                Follow entities to see them here.
              </p>
            )}
          </>
        )}
